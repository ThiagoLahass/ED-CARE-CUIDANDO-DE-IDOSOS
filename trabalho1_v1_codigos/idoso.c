...
double Calcula_distancia( float x1, float y1, float x2, float y2 );
...
void Idoso_faleceu( Idoso* idoso ){
    ...
    for( int i = 0; i < num_amigos; i++ ){
        ...
        Lista_idoso_remove( idoso->lista_amigos, Retorna_nome_idoso( amigo ));
        Lista_idoso_remove( amigo->lista_amigos, Retorna_nome_idoso( idoso ));
    } 
}
...
Lista_idoso* Idoso_retorna_lista_amigos( Idoso* idoso ) {
    return idoso->lista_amigos;
}
...
Cuidador* Idoso_retorna_cuidador_mais_proximo( Idoso* idoso ){
    ...
    double distancia_atual = 0, menor_distancia = 1000000000;
    num_cuidadores = Lista_cuidador_num_elementos( idoso->lista_cuidadores );
    Cuidador* cuidador_atual, *cuidador_mais_proximo;

    for( int i = 0; i < num_cuidadores; i++ ){
        cuidador_atual = 
        Lista_cuidador_Retorna_cuidador_pelo_indice( Retorna_lista_cuidadores( idoso ), i);
